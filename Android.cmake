...
# Notes about java versions:
# For compilation, the highest supported version depends on what R8 supports
# which seems to be version 11 in build tools version 32.0.0
# For running some of the build tools however, a newer version seems to be required as
# older versions don't have to required signing algorithms
# so you should manually set the following variables to a newer version (e.g. JDK 17):
# - Java_JAVA_EXECUTABLE:FILEPATH=C:/jdk-17.0.2/bin/java.exe
# - Java_KEYTOOL:FILEPATH=C:/jdk-17.0.2/bin/keytool.exe
...
find_file(ANDROID_APKSIGNER_JAR NAMES "apksigner.jar" PATHS "${ANDROID_BUILD_TOOLS_PATH}/lib" REQUIRED NO_DEFAULT_PATH NO_SYSTEM_ENVIRONMENT_PATH NO_CMAKE_FIND_ROOT_PATH)
...
function(add_android_package)
  ...
  if(ANDROID_PLATFORM MATCHES "android-([0-9]+)")
    set(ANDROID_PLATFORM_NR ${CMAKE_MATCH_1})
    message(STATUS "  Targeting android version: ${ANDROID_PLATFORM_NR}")
  else()
    message(FATAL "  Invalid android platform format: ${ANDROID_PLATFORM}")
  endif()
  ...
  add_custom_command(
    ...
    COMMAND ${Java_JAVA_EXECUTABLE} -cp ${ANDROID_D8_JAR} com.android.tools.r8.D8 ${JAR_PATH} --min-api ${ANDROID_PLATFORM_NR} --output ${CLASSES_DEX_PATH}
    DEPENDS ${JAR_PATH}
    COMMENT "Dexing"
    USES_TERMINAL
  )
